...
from coaching_companion.models import Note
...
def test_note(unix_timestamp):
    ...
    assert dashboard.title == "Test Note Title"
    assert dashboard.type_ == "note"
...
def test_note_default_values(unix_timestamp):
    # Create an instance of Note without optional fields
    dashboard = Note(title="Test Dashboard", created_at=unix_timestamp)
...
def test_note_name_max_length():
    # Create an instance of Note with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Note.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Note.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Note.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(ValueError):
        Note.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Note.model_validate({"type_": long_name})
