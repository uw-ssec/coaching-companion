...
from coaching_companion.models import AuthUserSubmissions
...
def test_authusersubmissions(unix_timestamp):
    # Create an instance of AuthUserSubmissions
    dashboard = AuthUserSubmissions(
        created_at=unix_timestamp,
        created_by=int(12345678901234567890),
        title="Test AuthUserSubmissions Title",
        type_="authusersubmissions"
    )
...
def test_authusersubmissions_default_values(unix_timestamp):
    # Create an instance of AuthUserSubmissions without optional fields
    dashboard = AuthUserSubmissions(title="Test Dashboard", created_at=unix_timestamp)
...
def test_authusersubmissions_name_max_length():
    # Define the datetime string
    datetime_str = "2021-12-01T00:00:00Z"
    # Parse the datetime string into a datetime object
    datetime_obj = datetime.strptime(datetime_str, "%Y-%m-%dT%H:%M:%SZ")
    ...
    with pytest.raises(ValueError):
        AuthUserSubmissions.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
