...
from coaching_companion.models import DualLanguageHandbook
...
def test_duallanguagehandbook(unix_timestamp):
    # Create an instance of DualLanguageHandbook
    dashboard = DualLanguageHandbook.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test DualLanguageHandbook Title",
        "type_": "duallanguagehandbook"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test DualLanguageHandbook Title"
    assert dashboard.type_ == "duallanguagehandbook"
...
def test_duallanguagehandbook_default_values():
    # Create an instance of DualLanguageHandbook without optional fields
    dashboard = DualLanguageHandbook(title="Test Dashboard")
...
def test_duallanguagehandbook_name_max_length():
    # Create an instance of DualLanguageHandbook with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        DualLanguageHandbook.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        DualLanguageHandbook.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        DualLanguageHandbook.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        DualLanguageHandbook.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        DualLanguageHandbook.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        DualLanguageHandbook.model_validate({"type_": long_name})
