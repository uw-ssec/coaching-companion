...
from coaching_companion.models import Limit
...
def test_limit_default_values():
    # Create an instance of Limit without optional fields
    dashboard = Limit(title="Test Dashboard")
...
def test_limit_name_max_length():
    # Create an instance of Limit with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Limit.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Limit.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Limit.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Limit.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Limit.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Limit.model_validate({"type_": long_name})
