...
from coaching_companion.models import CollaboraEndpoint
...
def test_collaboraendpoint(unix_timestamp):
    ...
    assert dashboard.title == "Test CollaboraEndpoint Title"
    assert dashboard.type_ == "collaboraendpoint"
...
def test_collaboraendpoint_default_values(unix_timestamp):
    # Create an instance of CollaboraEndpoint without optional fields
    dashboard = CollaboraEndpoint(title="Test Dashboard", created_at=unix_timestamp)
...
def test_collaboraendpoint_name_max_length():
    # Define the datetime string
    datetime_str = "2021-12-01T00:00:00Z"
    # Parse the datetime string into a datetime object
    datetime_obj = datetime.strptime(datetime_str, "%Y-%m-%dT%H:%M:%SZ")
    ...
    # Create an instance of CollaboraEndpoint with a name exceeding max_length
    long_name = "A" * 300
    # Validate the id
    with pytest.raises(ValueError):
        CollaboraEndpoint.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        CollaboraEndpoint.model_validate({"title": long_name})
    ...
    # Validate the type_
    with pytest.raises(ValueError):
        CollaboraEndpoint.model_validate({"type_": long_name})
