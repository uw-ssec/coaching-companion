...
from coaching_companion.models import SharedAssets
...
def test_sharedassets(unix_timestamp):
    # Create an instance of SharedAssets
    dashboard = SharedAssets.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test SharedAssets Title",
        "type_": "sharedassets"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test SharedAssets Title"
    assert dashboard.type_ == "sharedassets"
...
def test_sharedassets_default_values():
    # Create an instance of SharedAssets without optional fields
    dashboard = SharedAssets(title="Test Dashboard")
...
def test_sharedassets_name_max_length():
    # Create an instance of SharedAssets with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        SharedAssets.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        SharedAssets.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        SharedAssets.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        SharedAssets.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        SharedAssets.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        SharedAssets.model_validate({"type_": long_name})
