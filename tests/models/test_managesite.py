...
from coaching_companion.models import ManageSite
...
def test_managesite(unix_timestamp):
    # Create an instance of ManageSite
    dashboard = ManageSite.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ManageSite Title",
        "type_": "managesite"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageSite Title"
    assert dashboard.type_ == "managesite"
...
def test_managesite_default_values():
    # Create an instance of ManageSite without optional fields
    dashboard = ManageSite(title="Test Dashboard")
...
def test_managesite_name_max_length():
    # Create an instance of ManageSite with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ManageSite.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageSite.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageSite.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ManageSite.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ManageSite.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageSite.model_validate({"type_": long_name})
