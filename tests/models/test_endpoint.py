...
def test_endpoint(unix_timestamp):
    # Create an instance of Endpoint
    dashboard = Endpoint.model_validate(
...
def test_endpoint_default_values():
    ...
    dashboard = Endpoint(title="Test Dashboard")
...
def test_endpoint_name_max_length():
    ...
    with pytest.raises(ValueError):
        Endpoint.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
    ...
    with pytest.raises(OverflowError):
        Endpoint.model_validate({"created_at": long_num})
    # Validate the created_at
