...
from coaching_companion.models import UserInvitations
...
def test_userinvitations(unix_timestamp):
    # Create an instance of UserInvitations
    dashboard = UserInvitations.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test UserInvitations Title",
        "type_": "userinvitations"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test UserInvitations Title"
    assert dashboard.type_ == "userinvitations"
...
def test_userinvitations_default_values():
    # Create an instance of UserInvitations without optional fields
    dashboard = UserInvitations(title="Test Dashboard")
...
def test_userinvitations_name_max_length():
    # Create an instance of UserInvitations with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserInvitations.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserInvitations.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        UserInvitations.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        UserInvitations.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        UserInvitations.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        UserInvitations.model_validate({"type_": long_name})
