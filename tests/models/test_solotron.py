...
from coaching_companion.models import Solotron
...
def test_solotron(unix_timestamp):
    # Create an instance of Solotron
    dashboard = Solotron.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test Solotron Title",
        "type_": "solotron"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test Solotron Title"
    assert dashboard.type_ == "solotron"
...
def test_solotron_default_values():
    # Create an instance of Solotron without optional fields
    dashboard = Solotron(title="Test Dashboard")
...
def test_solotron_name_max_length():
    # Create an instance of Solotron with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Solotron.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Solotron.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Solotron.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Solotron.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Solotron.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Solotron.model_validate({"type_": long_name})
