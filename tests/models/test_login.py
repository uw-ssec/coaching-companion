...
from coaching_companion.models import Login
...
def test_login(unix_timestamp):
    # Create an instance of Login
    dashboard = Login.model_validate(
    ...
    assert dashboard.title == "Test Login Title"
    assert dashboard.type_ == "login"
...
def test_login_default_values():
    # Create an instance of Login without optional fields
    dashboard = Login(title="Test Dashboard")
...
def test_login_name_max_length():
    # Create an instance of Login with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Login.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Login.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Login.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Login.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Login.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Login.model_validate({"type_": long_name})
