...
from coaching_companion.models import MeritEndpoint
...
def test_meritendpoint(unix_timestamp):
    # Create an instance of MeritEndpoint
    dashboard = MeritEndpoint.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test MeritEndpoint Title",
        "type_": "meritendpoint"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test MeritEndpoint Title"
    assert dashboard.type_ == "meritendpoint"
...
def test_meritendpoint_default_values():
    # Create an instance of MeritEndpoint without optional fields
    dashboard = MeritEndpoint(title="Test Dashboard")
...
def test_meritendpoint_name_max_length():
    # Create an instance of MeritEndpoint with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        MeritEndpoint.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        MeritEndpoint.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        MeritEndpoint.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        MeritEndpoint.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        MeritEndpoint.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        MeritEndpoint.model_validate({"type_": long_name})
