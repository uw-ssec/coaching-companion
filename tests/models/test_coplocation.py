...
from coaching_companion.models import CopLocation
...
def test_coplocation(unix_timestamp):
    # Create an instance of CopLocation
    dashboard = CopLocation.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test CopLocation Title",
        "type_": "coplocation"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test CopLocation Title"
    assert dashboard.type_ == "coplocation"
...
def test_coplocation_default_values():
    # Create an instance of CopLocation without optional fields
    dashboard = CopLocation(title="Test Dashboard")
...
def test_coplocation_name_max_length():
    # Create an instance of CopLocation with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        CopLocation.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        CopLocation.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        CopLocation.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        CopLocation.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        CopLocation.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        CopLocation.model_validate({"type_": long_name})
