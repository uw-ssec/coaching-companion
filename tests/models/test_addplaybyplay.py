...
from coaching_companion.models import AddPlayByPlay
...
def test_addplaybyplay(unix_timestamp):
    # Create an instance of AddPlayByPlay
    dashboard = AddPlayByPlay.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AddPlayByPlay Title",
        "type_": "addplaybyplay"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AddPlayByPlay Title"
    assert dashboard.type_ == "addplaybyplay"
...
def test_addplaybyplay_default_values():
    # Create an instance of AddPlayByPlay without optional fields
    dashboard = AddPlayByPlay(title="Test Dashboard")
...
def test_addplaybyplay_name_max_length():
    # Create an instance of AddPlayByPlay with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AddPlayByPlay.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AddPlayByPlay.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AddPlayByPlay.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AddPlayByPlay.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AddPlayByPlay.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AddPlayByPlay.model_validate({"type_": long_name})
