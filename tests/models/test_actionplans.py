...
def test_actionplans(unix_timestamp):
    # Create an instance of ActionPlans
    dashboard = ActionPlans.model_validate(
    ...
    assert dashboard.title == "Test ActionPlans Title"
...
def test_actionplans_default_values():
    ...
    dashboard = ActionPlans(title="Test Dashboard")
...
def test_actionplans_name_max_length():
    ...
    with pytest.raises(ValueError):
        ActionPlans.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ActionPlans.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ActionPlans.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ActionPlans.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ActionPlans.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ActionPlans.model_validate({"type_": long_name})
