...
from coaching_companion.models import ResourceLibrary
...
def test_resourcelibrary(unix_timestamp):
    # Create an instance of ResourceLibrary
    dashboard = ResourceLibrary.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ResourceLibrary Title",
        "type_": "resourcelibrary"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ResourceLibrary Title"
    assert dashboard.type_ == "resourcelibrary"
...
def test_resourcelibrary_default_values():
    # Create an instance of ResourceLibrary without optional fields
    dashboard = ResourceLibrary(title="Test Dashboard")
...
def test_resourcelibrary_name_max_length():
    # Create an instance of ResourceLibrary with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ResourceLibrary.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ResourceLibrary.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ResourceLibrary.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ResourceLibrary.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ResourceLibrary.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ResourceLibrary.model_validate({"type_": long_name})
