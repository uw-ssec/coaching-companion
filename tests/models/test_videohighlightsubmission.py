...
from coaching_companion.models import VideoHighlightSubmission
...
def test_videohighlightsubmission(unix_timestamp):
    # Create an instance of VideoHighlightSubmission
    dashboard = VideoHighlightSubmission.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test VideoHighlightSubmission Title",
        "type_": "videohighlightsubmission"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test VideoHighlightSubmission Title"
    assert dashboard.type_ == "videohighlightsubmission"
...
def test_videohighlightsubmission_default_values():
    # Create an instance of VideoHighlightSubmission without optional fields
    dashboard = VideoHighlightSubmission(title="Test Dashboard")
...
def test_videohighlightsubmission_name_max_length():
    # Create an instance of VideoHighlightSubmission with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        VideoHighlightSubmission.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        VideoHighlightSubmission.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        VideoHighlightSubmission.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        VideoHighlightSubmission.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        VideoHighlightSubmission.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        VideoHighlightSubmission.model_validate({"type_": long_name})
