...
def test_awsdashboard(unix_timestamp):
    # Create an instance of AWSDashboard
    dashboard = AWSDashboard.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AWSDashboard Title",
        "type_": "awsdashboard"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AWSDashboard Title"
    assert dashboard.type_ == "awsdashboard"
...
def test_awsdashboard_default_values():
    ...
    dashboard = AWSDashboard(title="Test Dashboard")
...
def test_awsdashboard_name_max_length():
    ...
    with pytest.raises(ValueError):
        AWSDashboard.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AWSDashboard.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AWSDashboard.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AWSDashboard.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AWSDashboard.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AWSDashboard.model_validate({"type_": long_name})
