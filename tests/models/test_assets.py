...
from coaching_companion.models import Assets
...
def test_assets(unix_timestamp):
    # Create an instance of Assets
    dashboard = Assets.model_validate(
    ...
    assert dashboard.title == "Test Assets Title"
    assert dashboard.type_ == "assets"
...
def test_assets_default_values():
    # Create an instance of Assets without optional fields
    dashboard = Assets(title="Test Dashboard")
...
def test_assets_name_max_length():
    # Create an instance of Assets with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Assets.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Assets.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Assets.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Assets.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Assets.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Assets.model_validate({"type_": long_name})
