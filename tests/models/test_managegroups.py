...
from coaching_companion.models import ManageGroups
...
def test_managegroups(unix_timestamp):
    # Create an instance of ManageGroups
    dashboard = ManageGroups.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ManageGroups Title",
        "type_": "managegroups"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageGroups Title"
    assert dashboard.type_ == "managegroups"
...
def test_managegroups_default_values():
    # Create an instance of ManageGroups without optional fields
    dashboard = ManageGroups(title="Test Dashboard")
...
def test_managegroups_name_max_length():
    # Create an instance of ManageGroups with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ManageGroups.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageGroups.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageGroups.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ManageGroups.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ManageGroups.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageGroups.model_validate({"type_": long_name})
