...
from coaching_companion.models import SharedGoals
...
def test_sharedgoals(unix_timestamp):
    # Create an instance of SharedGoals
    dashboard = SharedGoals.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test SharedGoals Title",
        "type_": "sharedgoals"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test SharedGoals Title"
    assert dashboard.type_ == "sharedgoals"
...
def test_sharedgoals_default_values():
    # Create an instance of SharedGoals without optional fields
    dashboard = SharedGoals(title="Test Dashboard")
...
def test_sharedgoals_name_max_length():
    # Create an instance of SharedGoals with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        SharedGoals.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        SharedGoals.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        SharedGoals.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        SharedGoals.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        SharedGoals.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        SharedGoals.model_validate({"type_": long_name})
