...
from coaching_companion.models import Annotations
...
def test_annotations(unix_timestamp):
    # Create an instance of Annotations
    dashboard = Annotations.model_validate(
    ...
    assert dashboard.title == "Test Annotations Title"
    assert dashboard.type_ == "annotations"
...
def test_annotations_default_values():
    # Create an instance of Annotations without optional fields
    dashboard = Annotations(title="Test Dashboard")
...
def test_annotations_name_max_length():
    # Create an instance of Annotations with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Annotations.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Annotations.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Annotations.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Annotations.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Annotations.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Annotations.model_validate({"type_": long_name})
