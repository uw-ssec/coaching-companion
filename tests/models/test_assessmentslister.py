...
from coaching_companion.models import AssessmentsLister
...
def test_assessmentslister(unix_timestamp):
    # Create an instance of AssessmentsLister
    dashboard = AssessmentsLister.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AssessmentsLister Title",
        "type_": "assessmentslister"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AssessmentsLister Title"
    assert dashboard.type_ == "assessmentslister"
...
def test_assessmentslister_default_values():
    # Create an instance of AssessmentsLister without optional fields
    dashboard = AssessmentsLister(title="Test Dashboard")
...
def test_assessmentslister_name_max_length():
    # Create an instance of AssessmentsLister with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AssessmentsLister.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AssessmentsLister.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AssessmentsLister.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AssessmentsLister.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AssessmentsLister.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AssessmentsLister.model_validate({"type_": long_name})
