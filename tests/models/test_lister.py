...
from coaching_companion.models import Lister
...
def test_lister(unix_timestamp):
    # Create an instance of Lister
    dashboard = Lister.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test Lister Title",
        "type_": "lister"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test Lister Title"
    assert dashboard.type_ == "lister"
...
def test_lister_default_values():
    # Create an instance of Lister without optional fields
    dashboard = Lister(title="Test Dashboard")
...
def test_lister_name_max_length():
    # Create an instance of Lister with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Lister.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Lister.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Lister.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Lister.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Lister.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Lister.model_validate({"type_": long_name})
