...
from coaching_companion.models import AssessmentsEvaluation
...
def test_assessmentsevaluation(unix_timestamp):
    # Create an instance of AssessmentsEvaluation
    dashboard = AssessmentsEvaluation.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AssessmentsEvaluation Title",
        "type_": "assessmentsevaluation"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AssessmentsEvaluation Title"
    assert dashboard.type_ == "assessmentsevaluation"
...
def test_assessmentsevaluation_default_values():
    # Create an instance of AssessmentsEvaluation without optional fields
    dashboard = AssessmentsEvaluation(title="Test Dashboard")
...
def test_assessmentsevaluation_name_max_length():
    # Create an instance of AssessmentsEvaluation with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AssessmentsEvaluation.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AssessmentsEvaluation.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AssessmentsEvaluation.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AssessmentsEvaluation.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AssessmentsEvaluation.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AssessmentsEvaluation.model_validate({"type_": long_name})
