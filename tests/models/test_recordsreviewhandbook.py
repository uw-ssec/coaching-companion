...
from coaching_companion.models import RecordsReviewHandbook
...
def test_recordsreviewhandbook(unix_timestamp):
    # Create an instance of RecordsReviewHandbook
    dashboard = RecordsReviewHandbook.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test RecordsReviewHandbook Title",
        "type_": "recordsreviewhandbook"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test RecordsReviewHandbook Title"
    assert dashboard.type_ == "recordsreviewhandbook"
...
def test_recordsreviewhandbook_default_values():
    # Create an instance of RecordsReviewHandbook without optional fields
    dashboard = RecordsReviewHandbook(title="Test Dashboard")
...
def test_recordsreviewhandbook_name_max_length():
    # Create an instance of RecordsReviewHandbook with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        RecordsReviewHandbook.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        RecordsReviewHandbook.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        RecordsReviewHandbook.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        RecordsReviewHandbook.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        RecordsReviewHandbook.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        RecordsReviewHandbook.model_validate({"type_": long_name})
