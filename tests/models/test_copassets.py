...
from coaching_companion.models import CopAssets
...
def test_copassets(unix_timestamp):
    # Create an instance of CopAssets
    dashboard = CopAssets.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test CopAssets Title",
        "type_": "copassets"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test CopAssets Title"
    assert dashboard.type_ == "copassets"
...
def test_copassets_default_values():
    # Create an instance of CopAssets without optional fields
    dashboard = CopAssets(title="Test Dashboard")
...
def test_copassets_name_max_length():
    # Create an instance of CopAssets with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        CopAssets.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        CopAssets.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        CopAssets.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        CopAssets.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        CopAssets.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        CopAssets.model_validate({"type_": long_name})
