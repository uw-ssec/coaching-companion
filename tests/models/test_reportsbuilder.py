...
from coaching_companion.models import ReportsBuilder
...
def test_reportsbuilder(unix_timestamp):
    # Create an instance of ReportsBuilder
    dashboard = ReportsBuilder.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ReportsBuilder Title",
        "type_": "reportsbuilder"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ReportsBuilder Title"
    assert dashboard.type_ == "reportsbuilder"
...
def test_reportsbuilder_default_values():
    # Create an instance of ReportsBuilder without optional fields
    dashboard = ReportsBuilder(title="Test Dashboard")
...
def test_reportsbuilder_name_max_length():
    # Create an instance of ReportsBuilder with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ReportsBuilder.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ReportsBuilder.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ReportsBuilder.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ReportsBuilder.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ReportsBuilder.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ReportsBuilder.model_validate({"type_": long_name})
