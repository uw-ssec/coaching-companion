...
from coaching_companion.models import Grade
...
def test_grade(unix_timestamp):
    # Create an instance of Grade
    dashboard = Grade.model_validate(
    ...
    assert dashboard.title == "Test Grade Title"
    assert dashboard.type_ == "grade"
...
def test_grade_default_values():
    # Create an instance of Grade without optional fields
    dashboard = Grade(title="Test Dashboard")
...
def test_grade_name_max_length():
    # Create an instance of Grade with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Grade.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Grade.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Grade.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Grade.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Grade.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Grade.model_validate({"type_": long_name})
