...
from coaching_companion.models import DualLanguageSubmission
...
def test_duallanguagesubmission(unix_timestamp):
    # Create an instance of DualLanguageSubmission
    dashboard = DualLanguageSubmission.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test DualLanguageSubmission Title",
        "type_": "duallanguagesubmission"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test DualLanguageSubmission Title"
    assert dashboard.type_ == "duallanguagesubmission"
...
def test_duallanguagesubmission_default_values():
    # Create an instance of DualLanguageSubmission without optional fields
    dashboard = DualLanguageSubmission(title="Test Dashboard")
...
def test_duallanguagesubmission_name_max_length():
    # Create an instance of DualLanguageSubmission with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        DualLanguageSubmission.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        DualLanguageSubmission.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        DualLanguageSubmission.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        DualLanguageSubmission.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        DualLanguageSubmission.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        DualLanguageSubmission.model_validate({"type_": long_name})
