...
from coaching_companion.models import AssessmentsSite
...
def test_assessmentssite(unix_timestamp):
    # Create an instance of AssessmentsSite
    dashboard = AssessmentsSite.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AssessmentsSite Title",
        "type_": "assessmentssite"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AssessmentsSite Title"
    assert dashboard.type_ == "assessmentssite"
...
def test_assessmentssite_default_values():
    # Create an instance of AssessmentsSite without optional fields
    dashboard = AssessmentsSite(title="Test Dashboard")
...
def test_assessmentssite_name_max_length():
    # Create an instance of AssessmentsSite with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AssessmentsSite.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AssessmentsSite.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AssessmentsSite.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AssessmentsSite.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AssessmentsSite.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AssessmentsSite.model_validate({"type_": long_name})
