...
from coaching_companion.models import Links
...
def test_links(unix_timestamp):
    # Create an instance of Links
    dashboard = Links.model_validate(
    ...
    assert dashboard.title == "Test Links Title"
    assert dashboard.type_ == "links"
...
def test_links_default_values():
    # Create an instance of Links without optional fields
    dashboard = Links(title="Test Dashboard")
...
def test_links_name_max_length():
    # Create an instance of Links with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Links.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Links.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Links.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Links.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Links.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Links.model_validate({"type_": long_name})
