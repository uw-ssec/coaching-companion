...
from coaching_companion.models import ContactForm
...
def test_contactform(unix_timestamp):
    # Create an instance of ContactForm
    dashboard = ContactForm.model_validate(
    ...
    assert dashboard.title == "Test ContactForm Title"
    assert dashboard.type_ == "contactform"
...
def test_contactform_default_values():
    # Create an instance of ContactForm without optional fields
    dashboard = ContactForm(title="Test Dashboard")
...
def test_contactform_name_max_length():
    # Create an instance of ContactForm with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ContactForm.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ContactForm.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ContactForm.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ContactForm.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ContactForm.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ContactForm.model_validate({"type_": long_name})
