...
from coaching_companion.models import CoursesState
...
def test_coursesstate(unix_timestamp):
    # Create an instance of CoursesState
    dashboard = CoursesState.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test CoursesState Title",
        "type_": "coursesstate"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test CoursesState Title"
    assert dashboard.type_ == "coursesstate"
...
def test_coursesstate_default_values():
    # Create an instance of CoursesState without optional fields
    dashboard = CoursesState(title="Test Dashboard")
...
def test_coursesstate_name_max_length():
    # Create an instance of CoursesState with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        CoursesState.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        CoursesState.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        CoursesState.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        CoursesState.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        CoursesState.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        CoursesState.model_validate({"type_": long_name})
