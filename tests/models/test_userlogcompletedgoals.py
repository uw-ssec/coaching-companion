...
from coaching_companion.models import UserLogCompletedGoals
...
def test_userlogcompletedgoals(unix_timestamp):
    # Create an instance of UserLogCompletedGoals
    dashboard = UserLogCompletedGoals.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test UserLogCompletedGoals Title",
        "type_": "userlogcompletedgoals"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test UserLogCompletedGoals Title"
    assert dashboard.type_ == "userlogcompletedgoals"
...
def test_userlogcompletedgoals_default_values():
    # Create an instance of UserLogCompletedGoals without optional fields
    dashboard = UserLogCompletedGoals(title="Test Dashboard")
...
def test_userlogcompletedgoals_name_max_length():
    # Create an instance of UserLogCompletedGoals with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserLogCompletedGoals.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserLogCompletedGoals.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        UserLogCompletedGoals.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        UserLogCompletedGoals.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        UserLogCompletedGoals.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        UserLogCompletedGoals.model_validate({"type_": long_name})
