...
from coaching_companion.models import VideoConference
...
def test_videoconference(unix_timestamp):
    # Create an instance of VideoConference
    dashboard = VideoConference.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test VideoConference Title",
        "type_": "videoconference"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test VideoConference Title"
    assert dashboard.type_ == "videoconference"
...
def test_videoconference_default_values():
    # Create an instance of VideoConference without optional fields
    dashboard = VideoConference(title="Test Dashboard")
...
def test_videoconference_name_max_length():
    # Create an instance of VideoConference with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        VideoConference.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        VideoConference.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        VideoConference.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        VideoConference.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        VideoConference.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        VideoConference.model_validate({"type_": long_name})
