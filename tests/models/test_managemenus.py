...
from coaching_companion.models import ManageMenus
...
def test_managemenus(unix_timestamp):
    # Create an instance of ManageMenus
    dashboard = ManageMenus.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ManageMenus Title",
        "type_": "managemenus"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageMenus Title"
    assert dashboard.type_ == "managemenus"
...
def test_managemenus_default_values():
    # Create an instance of ManageMenus without optional fields
    dashboard = ManageMenus(title="Test Dashboard")
...
def test_managemenus_name_max_length():
    # Create an instance of ManageMenus with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ManageMenus.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageMenus.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageMenus.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ManageMenus.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ManageMenus.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageMenus.model_validate({"type_": long_name})
