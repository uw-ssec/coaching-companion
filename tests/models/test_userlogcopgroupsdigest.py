...
from coaching_companion.models import UserLogCopGroupsDigest
...
def test_userlogcopgroupsdigest(unix_timestamp):
    # Create an instance of UserLogCopGroupsDigest
    dashboard = UserLogCopGroupsDigest.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test UserLogCopGroupsDigest Title",
        "type_": "userlogcopgroupsdigest"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test UserLogCopGroupsDigest Title"
    assert dashboard.type_ == "userlogcopgroupsdigest"
...
def test_userlogcopgroupsdigest_default_values():
    # Create an instance of UserLogCopGroupsDigest without optional fields
    dashboard = UserLogCopGroupsDigest(title="Test Dashboard")
...
def test_userlogcopgroupsdigest_name_max_length():
    # Create an instance of UserLogCopGroupsDigest with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserLogCopGroupsDigest.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserLogCopGroupsDigest.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        UserLogCopGroupsDigest.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        UserLogCopGroupsDigest.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        UserLogCopGroupsDigest.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        UserLogCopGroupsDigest.model_validate({"type_": long_name})
