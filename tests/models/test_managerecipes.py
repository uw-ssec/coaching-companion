...
from coaching_companion.models import ManageRecipes
...
def test_managerecipes(unix_timestamp):
    # Create an instance of ManageRecipes
    dashboard = ManageRecipes.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ManageRecipes Title",
        "type_": "managerecipes"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageRecipes Title"
    assert dashboard.type_ == "managerecipes"
...
def test_managerecipes_default_values():
    # Create an instance of ManageRecipes without optional fields
    dashboard = ManageRecipes(title="Test Dashboard")
...
def test_managerecipes_name_max_length():
    # Create an instance of ManageRecipes with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ManageRecipes.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageRecipes.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageRecipes.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ManageRecipes.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ManageRecipes.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageRecipes.model_validate({"type_": long_name})
