...
from coaching_companion.models import Tagit
...
def test_tagit(unix_timestamp):
    # Create an instance of Tagit
    dashboard = Tagit.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test Tagit Title",
        "type_": "tagit"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test Tagit Title"
    assert dashboard.type_ == "tagit"
...
def test_tagit_default_values():
    # Create an instance of Tagit without optional fields
    dashboard = Tagit(title="Test Dashboard")
...
def test_tagit_name_max_length():
    # Create an instance of Tagit with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Tagit.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Tagit.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Tagit.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Tagit.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Tagit.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Tagit.model_validate({"type_": long_name})
