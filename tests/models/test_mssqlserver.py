...
from coaching_companion.models import MssqlServer
...
def test_mssqlserver(unix_timestamp):
    # Create an instance of MssqlServer
    dashboard = MssqlServer.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test MssqlServer Title",
        "type_": "mssqlserver"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test MssqlServer Title"
    assert dashboard.type_ == "mssqlserver"
...
def test_mssqlserver_default_values():
    # Create an instance of MssqlServer without optional fields
    dashboard = MssqlServer(title="Test Dashboard")
...
def test_mssqlserver_name_max_length():
    # Create an instance of MssqlServer with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        MssqlServer.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        MssqlServer.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        MssqlServer.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        MssqlServer.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        MssqlServer.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        MssqlServer.model_validate({"type_": long_name})
