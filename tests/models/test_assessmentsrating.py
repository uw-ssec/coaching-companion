...
from coaching_companion.models import AssessmentsRating
...
def test_assessmentsrating(unix_timestamp):
    # Create an instance of AssessmentsRating
    dashboard = AssessmentsRating.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AssessmentsRating Title",
        "type_": "assessmentsrating"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AssessmentsRating Title"
    assert dashboard.type_ == "assessmentsrating"
...
def test_assessmentsrating_default_values():
    # Create an instance of AssessmentsRating without optional fields
    dashboard = AssessmentsRating(title="Test Dashboard")
...
def test_assessmentsrating_name_max_length():
    # Create an instance of AssessmentsRating with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AssessmentsRating.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AssessmentsRating.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AssessmentsRating.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AssessmentsRating.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AssessmentsRating.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AssessmentsRating.model_validate({"type_": long_name})
