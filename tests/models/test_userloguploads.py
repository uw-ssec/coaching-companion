...
from coaching_companion.models import UserLogUploads
...
def test_userloguploads(unix_timestamp):
    # Create an instance of UserLogUploads
    dashboard = UserLogUploads.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test UserLogUploads Title",
        "type_": "userloguploads"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test UserLogUploads Title"
    assert dashboard.type_ == "userloguploads"
...
def test_userloguploads_default_values():
    # Create an instance of UserLogUploads without optional fields
    dashboard = UserLogUploads(title="Test Dashboard")
...
def test_userloguploads_name_max_length():
    # Create an instance of UserLogUploads with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserLogUploads.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserLogUploads.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        UserLogUploads.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        UserLogUploads.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        UserLogUploads.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        UserLogUploads.model_validate({"type_": long_name})
