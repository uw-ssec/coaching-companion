...
from coaching_companion.models import AuthUsersLogCompletedGoals
...
def test_authuserslogcompletedgoals(unix_timestamp):
    # Create an instance of AuthUsersLogCompletedGoals
    dashboard = AuthUsersLogCompletedGoals(
        created_at=unix_timestamp,
        created_by=int(12345678901234567890),
        title="Test AuthUsersLogCompletedGoals Title",
        type_="authuserslogcompletedgoals"
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AuthUsersLogCompletedGoals Title"
    assert dashboard.type_ == "authuserslogcompletedgoals"
...
def test_authuserslogcompletedgoals_default_values(unix_timestamp):
    # Create an instance of AuthUsersLogCompletedGoals without optional fields
    dashboard = AuthUsersLogCompletedGoals(title="Test Dashboard", created_at=unix_timestamp)
...
def test_authuserslogcompletedgoals_name_max_length():
    # Define the datetime string
    datetime_str = "2021-12-01T00:00:00Z"
    # Parse the datetime string into a datetime object
    datetime_obj = datetime.strptime(datetime_str, "%Y-%m-%dT%H:%M:%SZ")
    ...
    # Create an instance of AuthUsersLogCompletedGoals with a name exceeding max_length
    long_name = "A" * 300
    # Validate the id
    with pytest.raises(ValueError):
        AuthUsersLogCompletedGoals.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AuthUsersLogCompletedGoals.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AuthUsersLogCompletedGoals.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(ValueError):
        AuthUsersLogCompletedGoals.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AuthUsersLogCompletedGoals.model_validate({"type_": long_name})
