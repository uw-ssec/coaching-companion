...
from coaching_companion.models import Assignments
...
def test_assignments(unix_timestamp):
    # Create an instance of Assignments
    dashboard = Assignments.model_validate(
    ...
    assert dashboard.title == "Test Assignments Title"
    assert dashboard.type_ == "assignments"
...
def test_assignments_default_values():
    # Create an instance of Assignments without optional fields
    dashboard = Assignments(title="Test Dashboard")
...
def test_assignments_name_max_length():
    # Create an instance of Assignments with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Assignments.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Assignments.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Assignments.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Assignments.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Assignments.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Assignments.model_validate({"type_": long_name})
