...
from coaching_companion.models import Layout
...
def test_layout(unix_timestamp):
    # Create an instance of Layout
    dashboard = Layout.model_validate(
    ...
    assert dashboard.title == "Test Layout Title"
    assert dashboard.type_ == "layout"
...
def test_layout_default_values():
    # Create an instance of Layout without optional fields
    dashboard = Layout(title="Test Dashboard")
...
def test_layout_name_max_length():
    # Create an instance of Layout with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Layout.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Layout.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Layout.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Layout.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Layout.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Layout.model_validate({"type_": long_name})
