...
def test_eaevalsurvey(unix_timestamp):
    # Create an instance of EaEvalSurvey
    dashboard = EaEvalSurvey.model_validate(
    ...
    assert dashboard.title == "Test EaEvalSurvey Title"
...
def test_eaevalsurvey_default_values():
    ...
    dashboard = EaEvalSurvey(title="Test Dashboard")
...
def test_eaevalsurvey_name_max_length():
    ...
    with pytest.raises(ValueError):
        EaEvalSurvey.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        EaEvalSurvey.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        EaEvalSurvey.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        EaEvalSurvey.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        EaEvalSurvey.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        EaEvalSurvey.model_validate({"type_": long_name})
