...
from coaching_companion.models import UserSubmissions
...
def test_usersubmissions(unix_timestamp):
    # Create an instance of UserSubmissions
    dashboard = UserSubmissions.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test UserSubmissions Title",
        "type_": "usersubmissions"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test UserSubmissions Title"
    assert dashboard.type_ == "usersubmissions"
...
def test_usersubmissions_default_values():
    # Create an instance of UserSubmissions without optional fields
    dashboard = UserSubmissions(title="Test Dashboard")
...
def test_usersubmissions_name_max_length():
    # Create an instance of UserSubmissions with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserSubmissions.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserSubmissions.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        UserSubmissions.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        UserSubmissions.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        UserSubmissions.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        UserSubmissions.model_validate({"type_": long_name})
