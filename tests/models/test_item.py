...
from coaching_companion.models import Item
...
def test_item(unix_timestamp):
    ...
    assert dashboard.title == "Test Item Title"
    assert dashboard.type_ == "item"
...
def test_item_name_max_length():
    ...
    # Validate the title
    with pytest.raises(ValueError):
        Item.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Item.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Item.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Item.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Item.model_validate({"type_": long_name})
