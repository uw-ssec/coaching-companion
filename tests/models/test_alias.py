...
from coaching_companion.models import Alias
...
def test_alias_default_values():
    # Create an instance of Alias without optional fields
    dashboard = Alias(title="Test Dashboard")
...
def test_alias_name_max_length():
    # Create an instance of Alias with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Alias.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Alias.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Alias.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Alias.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Alias.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Alias.model_validate({"type_": long_name})
