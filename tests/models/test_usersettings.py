...
from coaching_companion.models import UserSettings
...
def test_usersettings(unix_timestamp):
    # Create an instance of UserSettings
    dashboard = UserSettings.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test UserSettings Title",
        "type_": "usersettings"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test UserSettings Title"
    assert dashboard.type_ == "usersettings"
...
def test_usersettings_default_values():
    # Create an instance of UserSettings without optional fields
    dashboard = UserSettings(title="Test Dashboard")
...
def test_usersettings_name_max_length():
    # Create an instance of UserSettings with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserSettings.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserSettings.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        UserSettings.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        UserSettings.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        UserSettings.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        UserSettings.model_validate({"type_": long_name})
