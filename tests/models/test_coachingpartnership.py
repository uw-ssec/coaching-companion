...
from coaching_companion.models import CoachingPartnership
...
def test_coachingpartnership(unix_timestamp):
    # Create an instance of CoachingPartnership
    dashboard = CoachingPartnership.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test CoachingPartnership Title",
        "type_": "coachingpartnership"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test CoachingPartnership Title"
    assert dashboard.type_ == "coachingpartnership"
...
def test_coachingpartnership_default_values():
    # Create an instance of CoachingPartnership without optional fields
    dashboard = CoachingPartnership(title="Test Dashboard")
...
def test_coachingpartnership_name_max_length():
    # Create an instance of CoachingPartnership with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        CoachingPartnership.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        CoachingPartnership.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        CoachingPartnership.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        CoachingPartnership.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        CoachingPartnership.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        CoachingPartnership.model_validate({"type_": long_name})
