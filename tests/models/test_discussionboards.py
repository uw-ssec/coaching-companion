...
from coaching_companion.models import DiscussionBoards
...
def test_discussionboards(unix_timestamp):
    # Create an instance of DiscussionBoards
    dashboard = DiscussionBoards.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test DiscussionBoards Title",
        "type_": "discussionboards"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test DiscussionBoards Title"
    assert dashboard.type_ == "discussionboards"
...
def test_discussionboards_default_values():
    # Create an instance of DiscussionBoards without optional fields
    dashboard = DiscussionBoards(title="Test Dashboard")
...
def test_discussionboards_name_max_length():
    # Create an instance of DiscussionBoards with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        DiscussionBoards.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        DiscussionBoards.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        DiscussionBoards.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        DiscussionBoards.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        DiscussionBoards.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        DiscussionBoards.model_validate({"type_": long_name})
