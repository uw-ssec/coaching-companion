...
from coaching_companion.models import ManageAuthUsers
...
def test_manageauthusers(unix_timestamp):
    # Create an instance of ManageAuthUsers
    dashboard = ManageAuthUsers(
        created_at=unix_timestamp,
        created_by=int(12345678901234567890),
        title="Test ManageAuthUsers Title",
        type_="manageauthusers"
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageAuthUsers Title"
    assert dashboard.type_ == "manageauthusers"
...
def test_manageauthusers_default_values(unix_timestamp):
    # Create an instance of ManageAuthUsers without optional fields
    dashboard = ManageAuthUsers(title="Test Dashboard", created_at=unix_timestamp)
...
def test_manageauthusers_name_max_length():
    # Define the datetime string
    datetime_str = "2021-12-01T00:00:00Z"
    # Parse the datetime string into a datetime object
    datetime_obj = datetime.strptime(datetime_str, "%Y-%m-%dT%H:%M:%SZ")
    ...
    # Create an instance of ManageAuthUsers with a name exceeding max_length
    long_name = "A" * 300
    # Validate the id
    with pytest.raises(ValueError):
        ManageAuthUsers.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageAuthUsers.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageAuthUsers.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(ValueError):
        ManageAuthUsers.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageAuthUsers.model_validate({"type_": long_name})
