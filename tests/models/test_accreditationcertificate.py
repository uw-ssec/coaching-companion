...
from coaching_companion.models import AccreditationCertificate
...
def test_accreditationcertificate(unix_timestamp):
    # Create an instance of AccreditationCertificate
    dashboard = AccreditationCertificate.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test AccreditationCertificate Title",
        "type_": "accreditationcertificate"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AccreditationCertificate Title"
    assert dashboard.type_ == "accreditationcertificate"
...
def test_accreditationcertificate_default_values():
    # Create an instance of AccreditationCertificate without optional fields
    dashboard = AccreditationCertificate(title="Test Dashboard")
...
def test_accreditationcertificate_name_max_length():
    # Create an instance of AccreditationCertificate with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AccreditationCertificate.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AccreditationCertificate.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AccreditationCertificate.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        AccreditationCertificate.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        AccreditationCertificate.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AccreditationCertificate.model_validate({"type_": long_name})
