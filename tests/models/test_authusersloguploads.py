...
from coaching_companion.models import AuthUsersLogUploads
...
def test_authusersloguploads(unix_timestamp):
    # Create an instance of AuthUsersLogUploads
    dashboard = AuthUsersLogUploads(
        created_at=unix_timestamp,
        created_by=int(12345678901234567890),
        title="Test AuthUsersLogUploads Title",
        type_="authusersloguploads"
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AuthUsersLogUploads Title"
    assert dashboard.type_ == "authusersloguploads"
...
def test_authusersloguploads_default_values(unix_timestamp):
    # Create an instance of AuthUsersLogUploads without optional fields
    dashboard = AuthUsersLogUploads(title="Test Dashboard", created_at=unix_timestamp)
...
def test_authusersloguploads_name_max_length():
    # Create an instance of AuthUsersLogUploads with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AuthUsersLogUploads.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AuthUsersLogUploads.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AuthUsersLogUploads.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(ValueError):
        AuthUsersLogUploads.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AuthUsersLogUploads.model_validate({"type_": long_name})
