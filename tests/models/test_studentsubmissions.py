...
from coaching_companion.models import StudentSubmissions
...
def test_studentsubmissions(unix_timestamp):
    # Create an instance of StudentSubmissions
    dashboard = StudentSubmissions.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test StudentSubmissions Title",
        "type_": "studentsubmissions"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test StudentSubmissions Title"
    assert dashboard.type_ == "studentsubmissions"
...
def test_studentsubmissions_default_values():
    # Create an instance of StudentSubmissions without optional fields
    dashboard = StudentSubmissions(title="Test Dashboard")
...
def test_studentsubmissions_name_max_length():
    # Create an instance of StudentSubmissions with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        StudentSubmissions.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        StudentSubmissions.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        StudentSubmissions.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        StudentSubmissions.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        StudentSubmissions.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        StudentSubmissions.model_validate({"type_": long_name})
