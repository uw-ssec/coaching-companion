...
from coaching_companion.models import ManageUsers
...
def test_manageusers(unix_timestamp):
    # Create an instance of ManageUsers
    dashboard = ManageUsers.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ManageUsers Title",
        "type_": "manageusers"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageUsers Title"
    assert dashboard.type_ == "manageusers"
...
def test_manageusers_default_values():
    # Create an instance of ManageUsers without optional fields
    dashboard = ManageUsers(title="Test Dashboard")
...
def test_manageusers_name_max_length():
    # Create an instance of ManageUsers with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ManageUsers.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageUsers.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageUsers.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ManageUsers.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ManageUsers.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageUsers.model_validate({"type_": long_name})
