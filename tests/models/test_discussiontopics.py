...
from coaching_companion.models import DiscussionTopics
...
def test_discussiontopics(unix_timestamp):
    # Create an instance of DiscussionTopics
    dashboard = DiscussionTopics.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test DiscussionTopics Title",
        "type_": "discussiontopics"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test DiscussionTopics Title"
    assert dashboard.type_ == "discussiontopics"
...
def test_discussiontopics_default_values():
    # Create an instance of DiscussionTopics without optional fields
    dashboard = DiscussionTopics(title="Test Dashboard")
...
def test_discussiontopics_name_max_length():
    # Create an instance of DiscussionTopics with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        DiscussionTopics.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        DiscussionTopics.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        DiscussionTopics.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        DiscussionTopics.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        DiscussionTopics.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        DiscussionTopics.model_validate({"type_": long_name})
