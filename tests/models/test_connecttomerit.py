...
from coaching_companion.models import ConnectToMerit
...
def test_connecttomerit(unix_timestamp):
    # Create an instance of ConnectToMerit
    dashboard = ConnectToMerit.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ConnectToMerit Title",
        "type_": "connecttomerit"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ConnectToMerit Title"
    assert dashboard.type_ == "connecttomerit"
...
def test_connecttomerit_default_values():
    # Create an instance of ConnectToMerit without optional fields
    dashboard = ConnectToMerit(title="Test Dashboard")
...
def test_connecttomerit_name_max_length():
    # Create an instance of ConnectToMerit with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ConnectToMerit.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ConnectToMerit.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ConnectToMerit.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ConnectToMerit.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ConnectToMerit.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ConnectToMerit.model_validate({"type_": long_name})
