...
from coaching_companion.models import UserLogAllAttributes
...
def test_userlogallattributes(unix_timestamp):
    ...
    assert dashboard.title == "Test UserLogAllAttributes Title"
    assert dashboard.type_ == "userlogallattributes"
...
def test_userlogallattributes_default_values():
    # Create an instance of UserLogAllAttributes without optional fields
    dashboard = UserLogAllAttributes(title="Test Dashboard")
...
def test_userlogallattributes_name_max_length():
    # Create an instance of UserLogAllAttributes with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        UserLogAllAttributes.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        UserLogAllAttributes.model_validate({"title": long_name})
    ...
    # Validate the type_
    with pytest.raises(ValueError):
        UserLogAllAttributes.model_validate({"type_": long_name})
