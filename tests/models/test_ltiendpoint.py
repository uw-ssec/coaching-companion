...
from coaching_companion.models import LtiEndpoint
...
def test_ltiendpoint(unix_timestamp):
    # Create an instance of LtiEndpoint
    dashboard = LtiEndpoint.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test LtiEndpoint Title",
        "type_": "ltiendpoint"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test LtiEndpoint Title"
    assert dashboard.type_ == "ltiendpoint"
...
def test_ltiendpoint_default_values():
    # Create an instance of LtiEndpoint without optional fields
    dashboard = LtiEndpoint(title="Test Dashboard")
...
def test_ltiendpoint_name_max_length():
    # Create an instance of LtiEndpoint with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        LtiEndpoint.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        LtiEndpoint.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        LtiEndpoint.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        LtiEndpoint.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        LtiEndpoint.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        LtiEndpoint.model_validate({"type_": long_name})
