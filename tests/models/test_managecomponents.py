...
from coaching_companion.models import ManageComponents
...
def test_managecomponents(unix_timestamp):
    # Create an instance of ManageComponents
    dashboard = ManageComponents.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ManageComponents Title",
        "type_": "managecomponents"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ManageComponents Title"
    assert dashboard.type_ == "managecomponents"
...
def test_managecomponents_default_values():
    # Create an instance of ManageComponents without optional fields
    dashboard = ManageComponents(title="Test Dashboard")
...
def test_managecomponents_name_max_length():
    # Create an instance of ManageComponents with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ManageComponents.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ManageComponents.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ManageComponents.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ManageComponents.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ManageComponents.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ManageComponents.model_validate({"type_": long_name})
