...
from coaching_companion.models import RatingsRubric
...
def test_ratingsrubric(unix_timestamp):
    # Create an instance of RatingsRubric
    dashboard = RatingsRubric.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test RatingsRubric Title",
        "type_": "ratingsrubric"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test RatingsRubric Title"
    assert dashboard.type_ == "ratingsrubric"
...
def test_ratingsrubric_default_values():
    # Create an instance of RatingsRubric without optional fields
    dashboard = RatingsRubric(title="Test Dashboard")
...
def test_ratingsrubric_name_max_length():
    # Create an instance of RatingsRubric with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        RatingsRubric.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        RatingsRubric.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        RatingsRubric.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        RatingsRubric.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        RatingsRubric.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        RatingsRubric.model_validate({"type_": long_name})
