...
from coaching_companion.models import FamilySurveyLink
...
def test_familysurveylink(unix_timestamp):
    # Create an instance of FamilySurveyLink
    dashboard = FamilySurveyLink.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test FamilySurveyLink Title",
        "type_": "familysurveylink"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test FamilySurveyLink Title"
    assert dashboard.type_ == "familysurveylink"
...
def test_familysurveylink_default_values():
    # Create an instance of FamilySurveyLink without optional fields
    dashboard = FamilySurveyLink(title="Test Dashboard")
...
def test_familysurveylink_name_max_length():
    # Create an instance of FamilySurveyLink with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        FamilySurveyLink.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        FamilySurveyLink.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        FamilySurveyLink.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        FamilySurveyLink.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        FamilySurveyLink.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        FamilySurveyLink.model_validate({"type_": long_name})
