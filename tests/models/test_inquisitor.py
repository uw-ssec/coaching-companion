...
from coaching_companion.models import Inquisitor
...
def test_inquisitor(unix_timestamp):
    # Create an instance of Inquisitor
    dashboard = Inquisitor.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test Inquisitor Title",
        "type_": "inquisitor"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test Inquisitor Title"
    assert dashboard.type_ == "inquisitor"
...
def test_inquisitor_default_values():
    # Create an instance of Inquisitor without optional fields
    dashboard = Inquisitor(title="Test Dashboard")
...
def test_inquisitor_name_max_length():
    # Create an instance of Inquisitor with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Inquisitor.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Inquisitor.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Inquisitor.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Inquisitor.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Inquisitor.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Inquisitor.model_validate({"type_": long_name})
