...
from coaching_companion.models import AuthUserSettings
...
def test_authusersettings(unix_timestamp):
    # Create an instance of AuthUserSettings
    dashboard = AuthUserSettings(
        created_at=unix_timestamp,
        created_by=int(12345678901234567890),
        title="Test AuthUserSettings Title",
        type_="authusersettings"
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test AuthUserSettings Title"
    assert dashboard.type_ == "authusersettings"
...
def test_authusersettings_default_values(unix_timestamp):
    # Create an instance of AuthUserSettings without optional fields
    dashboard = AuthUserSettings(title="Test Dashboard", created_at=unix_timestamp)
...
def test_authusersettings_name_max_length():
    # Create an instance of AuthUserSettings with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        AuthUserSettings.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        AuthUserSettings.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        AuthUserSettings.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(ValueError):
        AuthUserSettings.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        AuthUserSettings.model_validate({"type_": long_name})
