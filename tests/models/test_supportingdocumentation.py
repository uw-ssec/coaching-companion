...
from coaching_companion.models import SupportingDocumentation
...
def test_supportingdocumentation(unix_timestamp):
    # Create an instance of SupportingDocumentation
    dashboard = SupportingDocumentation.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test SupportingDocumentation Title",
        "type_": "supportingdocumentation"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test SupportingDocumentation Title"
    assert dashboard.type_ == "supportingdocumentation"
...
def test_supportingdocumentation_default_values():
    # Create an instance of SupportingDocumentation without optional fields
    dashboard = SupportingDocumentation(title="Test Dashboard")
...
def test_supportingdocumentation_name_max_length():
    # Create an instance of SupportingDocumentation with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        SupportingDocumentation.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        SupportingDocumentation.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        SupportingDocumentation.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        SupportingDocumentation.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        SupportingDocumentation.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        SupportingDocumentation.model_validate({"type_": long_name})
