...
def test_reports(unix_timestamp):
    ...
    dashboard = Reports.model_validate(
    ...
    assert dashboard.title == "Test Reports Title"
    assert dashboard.type_ == "reports"
...
def test_reports_default_values():
    ...
    dashboard = Reports(title="Test Dashboard")
...
def test_reports_name_max_length():
    ...
    with pytest.raises(ValueError):
        Reports.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Reports.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Reports.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Reports.model_validate({"type_": long_name})
