...
from coaching_companion.models import RecordsReviewSubmission
...
def test_recordsreviewsubmission(unix_timestamp):
    # Create an instance of RecordsReviewSubmission
    dashboard = RecordsReviewSubmission.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test RecordsReviewSubmission Title",
        "type_": "recordsreviewsubmission"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test RecordsReviewSubmission Title"
    assert dashboard.type_ == "recordsreviewsubmission"
...
def test_recordsreviewsubmission_default_values():
    # Create an instance of RecordsReviewSubmission without optional fields
    dashboard = RecordsReviewSubmission(title="Test Dashboard")
...
def test_recordsreviewsubmission_name_max_length():
    # Create an instance of RecordsReviewSubmission with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        RecordsReviewSubmission.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        RecordsReviewSubmission.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        RecordsReviewSubmission.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        RecordsReviewSubmission.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        RecordsReviewSubmission.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        RecordsReviewSubmission.model_validate({"type_": long_name})
