...
from coaching_companion.models import ResetPassword
...
def test_resetpassword(unix_timestamp):
    # Create an instance of ResetPassword
    dashboard = ResetPassword.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ResetPassword Title",
        "type_": "resetpassword"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ResetPassword Title"
    assert dashboard.type_ == "resetpassword"
...
def test_resetpassword_default_values():
    # Create an instance of ResetPassword without optional fields
    dashboard = ResetPassword(title="Test Dashboard")
...
def test_resetpassword_name_max_length():
    # Create an instance of ResetPassword with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ResetPassword.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ResetPassword.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ResetPassword.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ResetPassword.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ResetPassword.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ResetPassword.model_validate({"type_": long_name})
