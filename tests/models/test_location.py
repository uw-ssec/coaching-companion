...
from coaching_companion.models import Location
...
def test_location(unix_timestamp):
    # Create an instance of Location
    dashboard = Location.model_validate(
    ...
    assert dashboard.title == "Test Location Title"
    assert dashboard.type_ == "location"
...
def test_location_default_values():
    # Create an instance of Location without optional fields
    dashboard = Location(title="Test Dashboard")
...
def test_location_name_max_length():
    # Create an instance of Location with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Location.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Location.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Location.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Location.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Location.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Location.model_validate({"type_": long_name})
