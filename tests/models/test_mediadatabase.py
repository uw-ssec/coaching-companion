...
from coaching_companion.models import MediaDatabase
...
def test_mediadatabase(unix_timestamp):
    # Create an instance of MediaDatabase
    dashboard = MediaDatabase.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test MediaDatabase Title",
        "type_": "mediadatabase"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test MediaDatabase Title"
    assert dashboard.type_ == "mediadatabase"
...
def test_mediadatabase_default_values():
    # Create an instance of MediaDatabase without optional fields
    dashboard = MediaDatabase(title="Test Dashboard")
...
def test_mediadatabase_name_max_length():
    # Create an instance of MediaDatabase with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        MediaDatabase.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        MediaDatabase.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        MediaDatabase.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        MediaDatabase.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        MediaDatabase.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        MediaDatabase.model_validate({"type_": long_name})
