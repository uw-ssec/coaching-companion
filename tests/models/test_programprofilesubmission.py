...
from coaching_companion.models import ProgramProfileSubmission
...
def test_programprofilesubmission(unix_timestamp):
    # Create an instance of ProgramProfileSubmission
    dashboard = ProgramProfileSubmission.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test ProgramProfileSubmission Title",
        "type_": "programprofilesubmission"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test ProgramProfileSubmission Title"
    assert dashboard.type_ == "programprofilesubmission"
...
def test_programprofilesubmission_default_values():
    # Create an instance of ProgramProfileSubmission without optional fields
    dashboard = ProgramProfileSubmission(title="Test Dashboard")
...
def test_programprofilesubmission_name_max_length():
    # Create an instance of ProgramProfileSubmission with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        ProgramProfileSubmission.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        ProgramProfileSubmission.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        ProgramProfileSubmission.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        ProgramProfileSubmission.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        ProgramProfileSubmission.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        ProgramProfileSubmission.model_validate({"type_": long_name})
