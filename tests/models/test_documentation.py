...
from coaching_companion.models import Documentation
...
def test_documentation(unix_timestamp):
    # Create an instance of Documentation
    dashboard = Documentation.model_validate(
    ...
    assert dashboard.title == "Test Documentation Title"
    assert dashboard.type_ == "documentation"
...
def test_documentation_default_values():
    # Create an instance of Documentation without optional fields
    dashboard = Documentation(title="Test Dashboard")
...
def test_documentation_name_max_length():
    # Create an instance of Documentation with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        Documentation.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        Documentation.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        Documentation.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        Documentation.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        Documentation.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        Documentation.model_validate({"type_": long_name})
