...
from coaching_companion.models import CycleDashboard
...
def test_cycledashboard(unix_timestamp):
    # Create an instance of CycleDashboard
    dashboard = CycleDashboard.model_validate(
        {"created_at": unix_timestamp,
        "created_by": int(12345678901234567890),
        "title": "Test CycleDashboard Title",
        "type_": "cycledashboard"}
    )

    # Assert that the fields are correctly set
    assert dashboard.created_by == int(12345678901234567890)
    assert dashboard.title == "Test CycleDashboard Title"
    assert dashboard.type_ == "cycledashboard"
...
def test_cycledashboard_default_values():
    # Create an instance of CycleDashboard without optional fields
    dashboard = CycleDashboard(title="Test Dashboard")
...
def test_cycledashboard_name_max_length():
    # Create an instance of CycleDashboard with a name exceeding max_length
    long_name = "A" * 300 # For testing non-text type fields
    long_num = 12345678901234567890 # For testing text type fields
    # Validate the id
    with pytest.raises(ValueError):
        CycleDashboard.model_validate({"id": long_name})
    # Validate the title
    with pytest.raises(ValueError):
        CycleDashboard.model_validate({"title": long_name})
    # Validate the created_by
    with pytest.raises(ValueError):
        CycleDashboard.model_validate({"created_by": long_name})
    # Validate the created_at
    with pytest.raises(OverflowError):
        CycleDashboard.model_validate({"created_at": long_num})
    # Validate the created_at
    with pytest.raises(TypeError):
        CycleDashboard.model_validate({"created_at": long_name})
    # Validate the type_
    with pytest.raises(ValueError):
        CycleDashboard.model_validate({"type_": long_name})
